...
развлечений. В этой статье вы узнаете как при помощи OpenCV и языка программирования python распозновать жесты, 
...
После захвата, масштабируем изображение и отзеркалим его для нашего удобства
```python
...
```
Если угол острый, то значит, мы натолкнулись на палец; увеличиваем счетчик *fingers* на 1.
```python
